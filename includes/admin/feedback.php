...
class Feedback  {
  ...
  public function ajax_post_timeline_deactivate_feedback() {
    ...
    if ((isset($_POST['formData']))) {
      ...
      if ( ! isset( $_post['_wpnonce'] ) || ! wp_verify_nonce( $_post['_wpnonce'], '_post_timeline_deactivate_feedback_nonce' ) ) {
        wp_send_json_error();
      }
  ...
  public static function send_feedback( $feedback_key, $feedback_text ) {
    ...
    $subject  = esc_attr__("Deactivation FeedBack",'post-timeline');
  ...
  public function box_html() {
    ...
    ?>
    <div class="ptl-cont modal fade ptl-feedback-popup" id="ptl-feedback-popup" tabindex="-1" role="dialog" aria-labelledby="ptl-feedback-modal" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="ptl-feedback-modal">Feedback</h5>
            ...
            <form id="ptl-feedback-form" method="post">
              ...
              <?php foreach ( $deactivate_reasons as $reason_key => $reason ) : 
                ...
                <div class="form-group">
                  <input id="ptl-deactivate-feedback-<?php echo esc_attr( $reason_key ); ?>" <?php echo $def_checked; ?> class="ptl-deactivate-feedback-dialog-input" type="radio" name="reason_key" value="<?php echo esc_attr( $reason_key ); ?>" />
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" id="ptl-feedback-skip" data-dismiss="modal"><?php echo esc_attr__('Deactivate','post-timeline') ?></button>
            <button type="button" class="btn btn-primary" id="ptl-feedback-submit" data-dismiss="modal"><?php echo esc_attr__('Submit & Deactivate','post-timeline') ?></button>
          </div>
        </div>
      </div>
    </div>
  ...
  public function add_action_link( $links, $file ) {
    ...
    $settings_link = '<a href="' . esc_url( $settings_url ) . '">' . __( 'Settings', 'p_timeline' ) . '</a>';
    array_unshift( $links, $settings_link );

    $gopro_url = 'https://posttimeline.com';
    $gopro_link = '<a class="ptl-gopro" href="' . esc_url( $gopro_url ) . '" target="__blank">' . __( 'Go Pro', 'p_timeline' ) . '</a>';
