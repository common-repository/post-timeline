...
class Base {
		...
		if(isset($this->settings['ptl-fonts-title']) && isset($this->settings['ptl-fonts-title'])) {
			...
			if($font_name) {
				$font_url = 'https://fonts.googleapis.com/css?family='.$font_name.$font_vars.$font_subsets;
	  			wp_enqueue_style( 'post-timeline'.'-fonts', $font_url, array(), '', '' );
  			}
		}
	}
	...
	public function query_all_posts() {
		...
		if($ptl_settings['ptl-type'] == 'tag') {
			...
			if ($ptl_settings['ptl-term-csort'] == 'on') {
				...
				$query_args['orderby'] = 'ptl_tax_tag';
			}
		...
		// Sort by Date
		else if($ptl_settings['ptl-type'] == 'date') {

    	    $query_args['meta_query'] = array(
                'meta_query' => array(
			        'relation' => 'AND',
			        'post_date_clause' => array(
			            'key'     => 'ptl-post-date',
	                	'value'		=> '',
			            'compare' => '!=',
			        ),
			        'post_order_clause' => array(
			            'key'     => 'ptl-post-order',
			            'compare' => 'EXIST',
			        ), 
			    ),
	...
	public function categorize_posts($all_posts) {
		...
		foreach ($all_posts as $_post) {
			...
			//	Close the missing tags
			$_post->post_content = Helper::truncate($_post->post_content, $all_configs["ptl-post-length"]);


			if ((isset($all_configs["ptl-html"])) && $all_configs["ptl-html"] != 1) {
			...
			if($by_date) {
				...
				//	Full date
				$_post->event_date = date($this->settings['ptl-date-format'], strtotime($tl_date));
	...
	public function add_tag($tag, $color) {
		...
		$html .= '<div  class="ptl-row">';
		   $html .= '<div class="pol-md-12">';
		      ...
		      $html .= '<div  class="ptl-sec-year" data-id="'.$hash.'">'. $tag.'</div>';
		   $html .= '</div>';
		$html .= '</div>';
		// <!-- Year Row End -->
	...
	public function render_template() {
		...
		foreach ($all_posts as $tag => $post_list) {
			...
			foreach ($post_list as $post_index => $post) {
				...
				$html .=  $this->render_post($post, $is_even);
		...
		foreach ($all_posts as $tag => $post_list) {
			...
			foreach ($post_list as $post_index => $post) {
				...
				$is_even = ($post_index % 2 == 0) ? true: false;
			 	...
			 	$section .= $this->render_post($post, $is_even);
	  ...
	  $html .= '<div class="ptl-row">';
	     $html .= '<div class="pol-lg-11 pol-md-12">';
	        $html .= '<div class="ptl-tmpl-main my-5">';
	        	...
	        	if ($this->settings['ptl-nav-status'] == 'on') {
	        	...
	        	if ($this->settings['ptl-type'] == 'tag' && count($all_posts) <= 0) {
	        		...
	        		$html .= '<span class="ptl-center-line '. $this->settings['ptl-line-style'] .'"></span>';
	        	}
			...
			if (!empty($gallery)) {
				if ($this->settings['ptl-slides-setting'] == 'carousel') {
					...
					if ($img){
						...
						if ($this->settings['ptl-lazy-load'] == 'on') $img_src = 'src="'.$this->placeholder_img.'" data-src="'.$img.'"';
					...
					foreach ($gallery as $image) {
						if ($image) {
							...
							if ($this->settings['ptl-lazy-load'] == 'on') $img_src = 'src="'.$this->placeholder_img.'" data-src="'.$url.'"';
   ...
   */
	public function social_icons($post_id) {
		$html = '';
			...
			foreach ($this->settings['social-shares'] as $key => $value) {
				...
				$links = array(
					array('facebook' => '//www.facebook.com/sharer/sharer.php?u=','icon'=>'facebook-f'),
					array('twitter' => '//twitter.com/intent/tweet?text=PostTimeLine&amp;url=','icon'=>'twitter'),
					array('linkedin' => '//www.linkedin.com/','icon'=>'linkedin-in'),
					array('pinterest' => '//plus.google.com/share?url=','icon'=>'pinterest'),

				);

				if ($value == 'on') {
	
					$html .= '<li><a href="javascript:void(0);" class="button" data-sharer="'.$key.'" data-title="'.get_the_title($post_id).'" data-url="'.$links[$repeat_counter][$key].get_permalink($post_id).'"><i class="fa-brands fa-'.$links[$repeat_counter]['icon'].'"></i></a></li>';
				}			
	...
	private function get_post_tag($post_id) {
	    ...
	    $taxonomy_names = wp_get_object_terms($post_id, 'ptl_tag',  array("fields" => "names")); 
