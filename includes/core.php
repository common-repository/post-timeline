...
class Core {
	...
	*/
	...
	global $post;
		...
		if ($post) {
			...
			{
				echo '<a href="#" id="ptl-shortcode-insert" data-toggle="popout" data-target="#insert-ptl-shortcode" class="button">Add Post-timeline Shortcode</a>';
			}
		}
	...
	*/
	public function shortcode_gen_popup() {
		...
		$ptl_navs =  array(
		  array('id' => '0',   'navigation_name' => 'Style 0', 'image' => POST_TIMELINE_URL_PATH . 'admin/images/layout/nav-0.png'),
		);
		...
		$def_skin = (isset(get_option('post_timeline_global_settings')['ptl-skin-type']) ? get_option('post_timeline_global_settings')['ptl-skin-type'] : '');
			...
			if ($post) {
				...
				if(in_array($post->post_type, ['page', 'post'])  && in_array($pagenow, ['post.php', 'post-new.php']))
				{
	...
	*/
	...
	public function ptl_custom_filter_admin(){
	  ...
	  if( $post_type == 'post-timeline' ) {
	  	...
	  	$terms_cat = get_terms( array(
	    	...
	    	$selected_cat = (isset($_GET['ptl_cat_filter'])) ? $_GET['ptl_cat_filter'] : ((isset($_GET['ptl_categories']))  ?  $_GET['ptl_categories'] : "");
	      echo '<select name="ptl_cat_filter">';
	    	...
	    	$terms_tag = get_terms( array(
	      	...
	      	$selected_tag = (isset($_GET['ptl_tag_filter'])) ? $_GET['ptl_tag_filter'] : ((isset($_GET['ptl_tag']))  ?  $_GET['ptl_tag'] : "");
	        ...
	        echo '<select name="ptl_tag_filter">';
	      	...
	      	$date = ( isset( $_GET['ptl_event'] ) && $_GET['ptl_event'] ) ? $_GET['ptl_event'] : '';
	      	...
	      	echo '<div class="input-group post_field date ptl-filter-date-picker ptl-cont" id="ptl-filter-date-picker">
                    <input type="text" class="form-control ptl-event-date-picker" name="ptl_event" id="ptl_event" value="'.$date.'"/>
                    <button class="ptl-date-clear" '.$cross.' type="button"><svg width="10" height="10" viewBox="0 0 12 12" xmlns="https://www.w3.org/2000/svg"><path d="M.566 1.698L0 1.13 1.132 0l.565.566L6 4.868 10.302.566 10.868 0 12 1.132l-.566.565L7.132 6l4.302 4.3.566.568L10.868 12l-.565-.566L6 7.132l-4.3 4.302L1.13 12 0 10.868l.566-.565L4.868 6 .566 1.698z"></path></svg></button>
	...
	public function ptl_custom_filter_list( $admin_query ) {
		...
		$post_type = (isset($_GET['post_type'])) ? sanitize_text_field($_GET['post_type']) : 'post-timeline';
